"""Generate PDFs for the DigiPermit submission document set (not the whole repo)."""
...
# The documents from the system-description request + submission bundle
SUBMISSION_DOCS = [
...
async def convert_one(page, markdown, src: Path, dest: Path) -> None:
...
async def main() -> None:
    ...
    print(f"Generating {len(SUBMISSION_DOCS)} submission PDF(s)...\n")
    ...
    async with async_playwright() as p:
        ...
        for name in SUBMISSION_DOCS:
            src = DOCS / name
            dest = OUT_DIR / name.replace(".md", ".pdf")
            if not src.exists():
                print(f"  SKIP  {name} (not found)")
                continue
            await convert_one(page, markdown, src, dest)
